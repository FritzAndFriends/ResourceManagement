...
namespace Fritz.ResourceManagement.Scheduling
{
    public class ScheduleManager
    {
		...
		private static (DateTime begin, DateTime end)? CalculateIntersectionOfTimeslots((DateTime StartDateTime, DateTime EndDateTime) ts1, (DateTime StartDateTime, DateTime EndDateTime) ts2) {

			// Console.Out.WriteLine($"Checking Time 1 {ts1} against Time 2 {ts2}");

			// Eliminate scenario 1 and 2
			if (!(ts1.StartDateTime <= ts2.EndDateTime && ts2.StartDateTime <= ts1.EndDateTime)) {
				// Console.Out.WriteLine("Did not find an intersection");
				return null;
			}

			// Scenario 3
			if (ts1.StartDateTime >= ts2.StartDateTime && ts1.EndDateTime <= ts2.EndDateTime) {
				// Console.Out.WriteLine("Time 2 is contained entirely within Time 1");
				return (ts1.StartDateTime, ts1.EndDateTime);
			}

			// Scenario 4
			if (ts1.StartDateTime <= ts2.StartDateTime && ts1.EndDateTime >= ts2.EndDateTime) {
				// Console.Out.WriteLine("Time 1 is contained entirely within Time 2");
				return (ts2.StartDateTime, ts2.EndDateTime);
			}

			// Scenario 5
			if (ts1.StartDateTime >= ts2.StartDateTime && ts1.EndDateTime >= ts2.EndDateTime) {
				// Console.Out.WriteLine("Time 1 starts after Time 2 and overlaps");
				return (ts1.StartDateTime, ts2.EndDateTime);
			}

			// Scenario 6
			if (ts1.StartDateTime <= ts2.StartDateTime && ts1.EndDateTime <= ts2.EndDateTime) {
				// Console.Out.WriteLine("Time 2 starts after Time 1 and overlaps");
				return (ts2.StartDateTime, ts1.EndDateTime);
			}

			// Console.Out.WriteLine("It broke");
			return null;
